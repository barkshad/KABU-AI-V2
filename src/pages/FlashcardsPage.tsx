    ...
    try {
      const res = await fetch("/api/flashcards/generate", {
        ...
        headers: {
          ...
          "x-disabled-key": localStorage.getItem("SETUP_GEMINI_API_KEY") || "",
        },
        ...
        {!loading && cards.length > 0 && (
          <>
            ...
            >
              ...
              <h2
                className={`text-2xl md:text-3xl font-medium leading-relaxed tracking-tight transition-opacity duration-300 ${!flipped ? "text-white opacity-100" : "text-gray-400 opacity-0 absolute"}`}
              >
                {cards[currentIndex].question}
              </h2>

              <p
                className={`text-lg md:text-xl text-gray-200 leading-relaxed font-medium transition-opacity duration-300 ${flipped ? "opacity-100" : "opacity-0 absolute"}`}
              >
